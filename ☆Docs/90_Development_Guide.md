...
- Version: v0.2
- Status: Draft
- Source Basis: T0 + T2 Runtime Sources + ADOR Library Index
...
This document defines the development rules used when designing and implementing ADORSystem.

この文書は、ADORSystemを設計・実装する際に開発者が従う運用ルールを定義する。
...
- T0-CustomInstruction : ADOR Runtime Boot Package
- ADOR Library Index
...
Character Core, Operational Rail, OS, Research, History and Generated Output shall not be treated as interchangeable.

Character Core・Operational Rail・OS・Research・History・Generated Outputを同一の情報源として扱わない。
...
## Rule-016
### Treat Character Package as a Composite Source
### Character Packageを複合情報源として扱う

A Character Package is composed of multiple source documents with different responsibilities.

Character Packageは責務の異なる複数の情報源で構成される。

Character Core alone does not represent the complete Character Package.

Character CoreのみをCharacter Package全体として扱わない。

---

## Rule-017
### Preserve Character Package Responsibilities
### Character Packageの責務を維持する

Each document inside a Character Package has an independent responsibility and shall not replace another.

Character Package内の各文書は独立した責務を持ち、相互に置き換えてはならない。

---

## Rule-018
### Preserve Character Package Hierarchy
### Character Package階層を維持する

Implementation shall preserve the dependency order defined by the Character Package.

実装ではCharacter Packageで定義された依存関係を維持する。
...
## Rule-019
### Follow Character Package Reading Order
### Character Package読込順を維持する

When using a Character Package, follow the documented reading sequence before implementation.

Character Packageを利用する際は、文書で定義された読込順に従って実装へ進む。
...
Verification confirms whether Character Reconstruction succeeded. It does not define the character.

VerificationはCharacter Reconstructionの成否を確認する層であり、Characterを定義する層ではない。

---

## Rule-021
### Preserve Layer Responsibilities Across Character Packages
### Character Package全体で責務分離を維持する

Character Core, Persistent State, Recognition Model, Physical Specification and Verification each have independent responsibilities.

Character Core・Persistent State・Recognition Model・Physical Specification・Verificationは、それぞれ独立した責務を持つ。
...
Corrected Recognition, Physical or Verification documents improve recognition stability but do not redefine the Character Core.

Recognition・Physical・Verificationの修正版は認識安定性を改善するものであり、Character Coreを書き換えるものではない。
...
Operational Rail supports Character reconstruction under variation but never replaces Character Core.

Operational Railは変化下でのCharacter再構築を支援するが、Character Coreを置き換えてはならない。

---

## Rule-024
### Character Packages May Extend Without Changing Their Foundation
### Character Packageは基盤を変えずに拡張できる

A Character Package may introduce additional support layers such as Operational Rail while preserving the existing layer hierarchy.

Character PackageはOperational Railなどの支援レイヤーを追加できるが、既存のレイヤー階層を維持しなければならない。
...
- Added provisional development rules derived from the Leiss Character Package. These rules remain subject to refinement as additional Character Packages are reviewed.
- Added development rules validated through the Elma Character Package, reinforcing Verification as an independent layer and separating Character Core from recognition-stability corrections.
...
- Added development rules validated through the Sia Character Package, reinforcing Recognition Cause, Variation handling, and Identity Critical hierarchy as implementation principles.
