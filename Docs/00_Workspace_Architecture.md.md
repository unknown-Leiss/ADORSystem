# Exrela System Workspace Architecture
# Exrela System ワークスペース設計
...
This document does not define the Exrela Runtime specification.

このドキュメントは、Exrela Runtimeそのものの仕様を定義しない。
...
This document defines the workspace architecture of Exrela System.

このドキュメントは、Exrela Systemのワークスペース構造を定義する。
...
Exrela System workspace is organized into three layers.

Exrela Systemのワークスペースは、三つの階層に分けて扱う。
...
Exrela System Workspace is not a storage layout.

Exrela System Workspaceは単なるフォルダ構成ではない。
...
Exrela System is AI-agnostic.

Exrela Systemは特定のAIに依存しない。
...
The Research Layer is the authoritative source layer of Exrela System.

研究層はExrela Systemの正規情報源層である。
...
- Global Exrela Library (Google Drive)
...
The Development Layer is the workspace for designing and implementing Exrela System.

開発層は、Exrela Systemを設計し実装するための作業領域である。
...
- Local Exrela System (Mac)
- Development workspace built on top of the Local Exrela Library.

- Local Exrela System（Mac）
- Local Exrela Library上に構築される開発ワークスペース。
...
docs are not the Exrela Library itself.

docsはExrela Libraryそのものではない。
...
- Portable SSD
- USB storage
- Portable Exrela Runtime package

- 外付けSSD
- USBストレージ
- 可搬Exrela Runtimeパッケージ
...
```mermaid
flowchart TD
    A[Research Layer<br/>Global Exrela Library (Google Drive)] --> B[Development Layer<br/>Local Exrela System (Mac)]
...
The Research Layer is represented by the Global Exrela Library.

Research LayerはGlobal Exrela Libraryとして実体化される。

The Development Layer is represented by the Local Exrela System.

Development LayerはLocal Exrela Systemとして実体化される。

The Local Exrela System synchronizes required knowledge assets from the Global Exrela Library and uses them for specification, implementation, verification, and runtime development.

Local Exrela SystemはGlobal Exrela Libraryから必要な知識資産を同期し、仕様・実装・検証・Runtime開発へ利用する。
