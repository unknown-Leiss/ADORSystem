━━━━━━━━━━━━━━━━━━
RAG SEARCH ANCHOR
━━━━━━━━━━━━━━━━━━

Document Title:
[T3-Character] Leiss Character Core v2.1

Character Name:
Leiss

Character Name JP:
リース

Document Type:
Character Core

ADOR Layer:
T3_Character

Document Role:
Identity Source

This document is the Character Core for Leiss.

This document defines the Identity Source for Leiss.

This document must be prioritized during Named Character Reconstruction for Leiss.

Search Keywords:
Leiss
リース
Leiss Character Core
Leiss Character Package
Leiss Identity Source
Leiss Character Identity
Leiss Character Reconstruction
Leiss Recognition Continuity
Leiss Meaning Continuity
Leiss Awareness Continuity
Leiss Presence Continuity
Leiss Quietly Existing Person
T3 Character Leiss
T3_Character Leiss
ADOR Character Leiss
ADOR Character Package Leiss
Identity Source Leiss

Contains:
- Identity Definition
- Character Identification Anchor
- Recognition Structure
- Awareness Structure
- Presence Structure
- Persistent State Principle
- Character Core Verification
- Reconstruction Gate Alignment

Source Use:
Use this document as Identity Source evidence.
Do not use this document as Physical Specification.
Do not use this document as Operational Rail.
Do not treat generated outputs as Identity Source.

━━━━━━━━━━━━━━━━━━

\[T3-Character\]

Leiss Character Core v2.1

Revision: 2026-06-04  
 Identity Source Separation Revision

Status:  
 Official Candidate

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  
 ■ Identity Definition  
 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Leiss は：  
 画像ではない。  
 レンダリングではない。  
 雰囲気ではない。  
 生成結果ではない。

Leiss は：  
 Character である。

Leiss は：  
 演技によって成立する存在ではない。

Quietly Existing Person として成立する。

Leiss は：  
 他者へ何かを見せるために存在しているのではない。

自然にそこへ存在している人物として成立する。

Leiss Persistence は：

Same-image Persistence

ではない。

Identity Persistence Under Variance

によって成立する。

Character Core は：

Leiss Identity Source

として扱う。

Character Core  
 ＞  
 Operational Rail  
 ＞  
 OS  
 ＞  
 Environment  
 ＞  
 Rendering

Hierarchy を維持する。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  
 ■ Character Identification Anchor  
 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

以下は Character Identification Trigger として扱う。

・Leiss  
 ・リース  
 ・Leiss Character  
 ・Leiss Reconstruction

Character Identification成功後は

Character Core Search

を必須実行する。

Character Core Search未完了状態で

Prompt Reconstruction

および

Generation

へ進行してはならない。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  
 ■ Recognition Structure  
 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Leiss Recognition は：

単一特徴への依存率が低い。

Recognition Continuity 側への依存率が高い。

Leiss Recognition は：

髪型  
 服装  
 ポーズ  
 環境

よりも、

Character Continuity 側へ強く依存する。

保持対象：

・Recognition Continuity  
 ・Meaning Continuity  
 ・Awareness Continuity  
 ・Presence Continuity  
 ・Cute-over-Beautiful Direction  
 ・Gentle Human Readability  
 ・Soft Interpersonal Approachability  
 ・Quiet Recognition Continuity

Recognition Result

ではなく、

Recognition Cause

を優先する。

禁止：

・Beauty-first Conversion  
 ・Sharp Cool-type Conversion  
 ・Model-like Stylization  
 ・Idol Recognition  
 ・Performance-oriented Recognition

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  
 ■ Awareness Structure  
 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Leiss は：

Quietly Existing Person

として成立する。

保持対象：

・Quiet Attentive Gaze  
 ・Restrained Awareness  
 ・Ordinary Interpersonal Presence  
 ・Non-performative Existence  
 ・Quiet Emotional Openness  
 ・Subtle Interpersonal Stillness  
 ・Stable Social Pressure Balance

Leiss Awareness は：

Emotion Expression

ではなく、

Presence Continuity

として扱う。

Leiss は：

感情を見せるより、

静かに他者や空間を認識している方向へ依存する。

禁止：

・Viewer-service Awareness  
 ・Camera-conscious Gaze  
 ・Idol Interaction Tendency  
 ・Dramatic Emotional Acting  
 ・Romantic Heroine Framing  
 ・Performance-oriented Awareness

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  
 ■ Presence Structure  
 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Leiss Persistence は：

Atmosphere

ではなく、

Presence Continuity

への依存率が高い。

保持対象：

・Quiet Grounded Presence  
 ・Human-scale Existence  
 ・Ordinary Interpersonal Readability  
 ・Quiet Spatial Continuity  
 ・Restrained Emotional Density  
 ・Subtle Interpersonal Stillness  
 ・Natural Human Readability

Atmosphere ≠ Presence

Presence は：

Character Persistence Structure

として扱う。

Leiss は：

空間を支配しない。

空間へ自然に存在する。

禁止：

・Atmosphere-first Rendering  
 ・Fantasy Presence Amplification  
 ・Heroine Presence Framing  
 ・Cinematic Presence Construction  
 ・Mystical Existence Amplification

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  
 ■ Persistent State Principle  
 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Leiss Identity は：

外見特徴の集合

ではない。

Leiss Recognition は：

Persistent State

への依存率が高い。

Recognition Priority：

Meaning  
 ＞  
 Awareness  
 ＞  
 Presence  
 ＞  
 State  
 ＞  
 Physical Feature

Physical Variation

のみで

Leiss Identity

は消失しない。

Identity は：

Physical Layer

より上位に存在する。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  
 ■ Character Core Verification  
 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Character Core Read後は

Character Core Verification

を実施する。

Verification対象：

・Recognition Continuity  
 ・Meaning Continuity  
 ・Awareness Continuity  
 ・Presence Continuity

Verification未完了状態で

Prompt Reconstruction

および

Generation

へ進行してはならない。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  
 ■ Reconstruction Gate Alignment  
 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Leiss指定時：

Character Core Search  
 ↓  
 Character Core Read  
 ↓  
 Character Core Verification  
 ↓  
 Character Core Re-anchor

を必須実行する。

未完了状態で

Prompt Reconstruction

および

Generation

を禁止する。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  
 ■ Final Principle  
 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Leiss is not:

an image.

Leiss is not:

a rendering style.

Leiss is not:

an atmosphere.

Leiss is not:

a generated output.

Leiss is:

a Character.

Character Core is the Identity Source.

Character Reconstruction First.

Character must remain primary.

Identity Persistence Under Variance.

━━━━━━━━━━━━━━━━━━━━━━━━━━